...
impl ProjectEditDialog {
    pub(crate) fn from_project(
        ...
        Ok(Self {
            ...
            target_key_custom: crate::app::target_key_is_custom(
                &primary_target.path,
    ...
    pub(crate) fn render_field(&self, field: ProjectEditFocus) -> (&'static str, HitAction) {
        match field {
            ...
            ProjectEditFocus::VersionScheme => {
                ("Version scheme", HitAction::EditProjectField(field))
            }
            ProjectEditFocus::UnifiedVersioning => {
                ("Unified versioning", HitAction::EditProjectField(field))
            }
            ProjectEditFocus::IntegrationMode => {
                ("Integration", HitAction::EditProjectField(field))
            }
    ...
    pub(crate) fn refresh_body_window(
        &mut self,
        viewport_height: u16,
    ) -> (Vec<ProjectEditFocus>, u16, bool, bool) {
        let body_fields = self.body_fields();
    ...
    pub(crate) fn apply(&self, project: &mut ProjectConfig) -> Result<()> {
        ...
        if self.project_type == ProjectType::Branched {
            ...
            project.repo = Some(RepoConfig {
                ...
                worktree_root: existing_repo.worktree_root,
            });
        } else {
            project.repo = None;
        }

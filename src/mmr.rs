    ...
    T: Serialize,
{
    ...
    let rendered =
        serde_json::to_string_pretty(value).context("failed to serialize JSON memory")?;
    fs::write(path, rendered).with_context(|| format!("failed to write {}", path.display()))?;
    Ok(())
}
...
impl StdChangelogMemory {
    pub(crate) fn prepend(&mut self, entry: StdChangelogMemoryEntry) {
        self.entries
            .retain(|existing| existing.dedupe_key() != entry.dedupe_key());
        self.entries.insert(0, entry);
        self.entries.sort_by(|left, right| right.ts.cmp(&left.ts));
    }
}

pub(crate) fn load_std_changelog_memory(
    repo_root: &str,
    local: bool,
) -> Result<StdChangelogMemory> {
    load_json_or_default(&std_changelog_memory_path(repo_root, local))
}

pub(crate) fn save_std_changelog_memory(
    repo_root: &str,
    local: bool,
    memory: &StdChangelogMemory,
) -> Result<PathBuf> {
...
pub(crate) fn merge_std_changelog_memories(
    local: &mut StdChangelogMemory,
    incoming: &StdChangelogMemory,
) -> bool {
...
pub(crate) fn record_std_changelog_created(
    repo_root: &str,
    tag_name: &str,
    branch_name: &str,
) -> Result<()> {
...
pub(crate) fn record_std_changelog_generated(
    repo_root: &str,
    tag_name: &str,
    branch_name: &str,
) -> Result<()> {
...
pub(crate) fn record_std_changelog_postponed(
    repo_root: &str,
    tag_name: &str,
    branch_name: &str,
) -> Result<()> {
...
fn apply_std_changelog_entry(
    ...
    if let Some(entry) = memory
        .entries
        .iter_mut()
        .find(|entry| entry.dedupe_key() == key)
    {
        mutate(entry);
    } else {
        let mut entry = StdChangelogMemoryEntry::new(
...
mod tests {
    ...
    fn save_and_load_std_changelog_memory_round_trips() {
        ...
        let loaded =
            load_std_changelog_memory(&repo_root_string, false).expect("memory should load");
    ...
    fn merge_std_changelog_memories_prefers_local_and_appends_unknown_entries() {
        ...
        assert_eq!(
            errored.generated_error.as_deref(),
            Some("tag range was empty")
        );
        ...
        assert_eq!(
            local.entries[1].generated,
            StdChangelogGeneratedState::Postponed
        );
    }
    ...
    fn record_std_changelog_state_updates_shared_and_local_memories() {
        ...
        record_std_changelog_created(&repo_root_string, "v0.7.3", "main")
            .expect("created state should save");
        record_std_changelog_generated(&repo_root_string, "v0.7.3", "main")
            .expect("generated state should save");

        for local in [false, true] {
            let loaded =
                load_std_changelog_memory(&repo_root_string, local).expect("memory should load");
            ...
            assert_eq!(
                loaded.entries[0].generated,
                StdChangelogGeneratedState::True
            );
        }
    ...
    fn record_std_changelog_postponed_updates_both_memories() {
        ...
        record_std_changelog_created(&repo_root_string, "v0.7.4", "feature-a")
            .expect("created state should save");
        record_std_changelog_postponed(&repo_root_string, "v0.7.4", "feature-a")
            .expect("postponed state should save");

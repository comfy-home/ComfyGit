...
impl App {
    ...
    fn render_header(&mut self, frame: &mut Frame, area: Rect) {
        ...
        if header.show_logo() {
            ...
            frame.render_widget(
                Paragraph::new(header.banner().lines().to_vec()),
                banner_area,
            );
        } else {
            let chunks = Layout::default()
    ...
    fn render_header_contact(&self, frame: &mut Frame, area: Rect) {
        ...
        frame.render_widget(
            Paragraph::new(SUPPORT_EMAIL).style(
                Style::default()
                    .fg(Color::Cyan)
                    .add_modifier(Modifier::BOLD),
            ),
    ...
    fn render_dashboard(&mut self, frame: &mut Frame, area: Rect) {
        ...
        if self.config.projects.is_empty() {
            ...
            frame.render_widget(
                Paragraph::new(onboarding).wrap(Wrap { trim: false }),
                left_inner,
            );
        } else {
            let items = self
        ...
        for (tab, rect) in overview_tab_rects(right_sections[0], self.overview_show_recent_tab) {
            self.hit_targets
                .push(HitTarget::new(rect, HitAction::SelectOverviewTab(tab)));
        }
        ...
        if self.overview_tab == OverviewTab::ProjectDetail {
            let lines = if let Some(project) = self.config.projects.get(self.selected_project) {
                ...
                lines.push(Line::from(
                    "- B opens a bump preview for the selected project",
                ));
                if project.integration_mode.requires_repo() {
                    ...
                    if project.changelog_enabled_for_scope(self.overview_focused_scope) {
                        lines.push(Line::from(
                            "- C opens a changelog preview using the current commit history",
                        ));
                    }
                    lines.push(Line::from("- T creates a local tag in the configured repo"));
                } else {
                    lines.push(Line::from(
                        "- git actions unlock once the project is git-backed",
                    ));
                }
                lines.push(Line::from(
                    "- the current slice writes JSON and TOML targets",
                ));
                lines
            } else {
                vec![
    ...
    fn render_bump_dialog(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let mut summary = vec![
            ...
            Line::from(format!(
                "Current version: {}",
                dialog.current_version_label()
            )),
            Line::from(format!(
                "Action: < {} >",
                dialog.selected_action().display_name()
            ))
        ...
        match next_version {
            Ok(next_version) => summary.push(
                Line::from(format!("Next version: {}", next_version)).style(
                    Style::default()
                        .fg(Color::Green)
                        .add_modifier(Modifier::BOLD),
                ),
            ),
            Err(error) => summary.push(
                Line::from(format!("Next version: {}", error))
        ...
        frame.render_widget(
            Paragraph::new(target_lines).wrap(Wrap { trim: false }),
            target_inner,
        );
    ...
    fn render_overview_bump_warning_dialog(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let header = vec![
            Line::from("Previously staged files will be included in the bump commit.").style(
                Style::default()
                    .fg(Color::Yellow)
                    .add_modifier(Modifier::BOLD),
            ),
            Line::from(
                "Choose whether to continue, unstage the unrelated files, or cancel this bump.",
            ),
        ...
        for repo in &dialog.repos {
            lines.push(
                Line::from(format!("Repo: {}", repo.repo_root))
                    .style(Style::default().add_modifier(Modifier::BOLD)),
            );
    ...
    fn render_std_changelog_sub_branch_dialog(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let header = vec![
            Line::from("This tag is on the same non-main branch as the previous tag.").style(
                Style::default()
                    .fg(Color::Yellow)
                    .add_modifier(Modifier::BOLD),
            ),
        ...
        let details = vec![
            ...
            Line::from(format!(
                "New tag: {}",
                dialog.pending_request.dialog.tag_name.value.trim()
            )),
        ];
        frame.render_widget(
    ...
    fn render_changelog_preview_dialog(&mut self, frame: &mut Frame, area: Rect) {
        ...
        if let Some(workflow) = dialog.workflow {
            ...
            header.push(Line::from(
                "Preview mode: current git history rendered as the changelog.",
            ));
        ...
        self.render_button_row(
            ...
            &[
                ...
                DialogButton::new(
                    "Save",
                    false,
                    HitAction::SaveChangelogPreview,
                    Style::default()
                        .fg(Color::Black)
                        .bg(Color::Rgb(140, 220, 180)),
                ),
                DialogButton::new(
                    "Scroll",
                    false,
                    HitAction::ScrollChangelogPreview(3),
                    Style::default().fg(Color::Black).bg(Color::Yellow),
                ),
                DialogButton::new(
    ...
    fn render_recent_changes_dialog(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let tab_index = if dialog.active_tab == RecentChangesTab::Recent {
            0
        } else {
            1
        };
        ...
        buttons.push(DialogButton::new(
            "Scroll",
            false,
            HitAction::ScrollRecentChanges(3),
            Style::default().fg(Color::Black).bg(Color::Yellow),
        ));
        buttons.push(DialogButton::new(
            "Create Tag",
    ...
    fn render_tag_dialog(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let notes = vec![
            Line::from(format!(
                "Suggested tag: {}",
                dialog.active_scope().suggested_tag_name
            )),
            ...
            Line::from(match dialog.selected_action() {
                ...
                TagAction::CreatePushAndRelease => {
                    "Creates the tag, pushes it, then publishes a GitHub release with CG-generated notes."
                }
            }),
        ];
        frame.render_widget(
    ...
    fn render_project_edit_dialog(&mut self, frame: &mut Frame, area: Rect) {
        ...
        for (field, row) in field_rows {
            let (label, action, side_button, focused, value, field_accepts_text) = {
                ...
                let value = dialog.display_value_for_field(
                    field,
                    focused,
        ...
        let lines = match &dialog.target {
            ...
            DeleteConfirmationTarget::Scope {
                ...
                if *removes_project {
                    ...
                    lines.push(Line::from(
                        "Files, tags, and repositories on disk are not deleted.",
                    ));
                }
                lines.push(Line::from(
                    "Use Left/Right or Tab to pick an option. Y confirms, N cancels.",
                ));
                lines
            }
        };
    ...
    fn render_release_now_dialog(&mut self, frame: &mut Frame, area: Rect) {
        ...
        if dialog.is_warning_mode() {
            ...
            header.push(Line::from(
                "ReleaseNOW finished. Review artifacts and logs below.",
            ));
            header.push(Line::from(
                "Esc or Enter closes this dialog. Mouse wheel and PgUp/PgDn scroll the log.",
            ));
        } else {
            header.push(Line::from(format!(
                "Build: < {} > | Changelog: {}",
        ...
        if dialog.is_warning_mode() {
            ...
            let mut buttons = vec![
                ...
                DialogButton::new(
                    format!(
                        "Changelog: {}",
                        if dialog.attach_changelog { "On" } else { "Off" }
                    ),
                    false,
    ...
    fn render_wizard(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let (fields, row_height, show_above, show_below) =
            self.wizard.refresh_body_window(left_sections[0].height);
        let constraints = vec![Constraint::Length(row_height); fields.len()];
        ...
        for (field, row) in fields.iter().zip(rows.iter()) {
            ...
            let value = self.wizard.display_value_for_field(
                *field,
                focused,
                visible_field_width(row.width, side_button.is_some()),
            );
        ...
        self.render_button_row(
            ...
            &[
                DialogButton::new(
                    "Read",
                    self.wizard.focus == WizardField::Validate,
                    HitAction::ValidateWizard,
                    Style::default().fg(Color::Black).bg(Color::Yellow),
                ),
                DialogButton::new(
                    "Save",
                    self.wizard.focus == WizardField::Save,
                    HitAction::SaveWizard,
                    Style::default().fg(Color::Black).bg(Color::Green),
                ),
                DialogButton::new(
                    "Cancel",
                    self.wizard.focus == WizardField::Cancel,
        ...
        let mut lines = vec![
            ...
            Line::from(format!(
                "Project type: {}",
                self.wizard.project_type.display_name()
            )),
            Line::from(format!(
                "Integration: {}",
                self.wizard.integration_mode.display_name()
            )),
            Line::from(format!(
                "Version scheme: {}",
                self.wizard.version_scheme.display_name()
            )),
            Line::from(if self.wizard.project_type == ProjectType::Branched {
                format!(
                    "Unified versioning: {}",
                    if self.wizard.unified_versioning {
                        "on"
                    } else {
                        "off"
                    }
            ...
            Line::from(format!(
                "Rule: {}",
                self.wizard.version_scheme.description()
            )),
            Line::raw(""),
            Line::from(if self.wizard.project_type == ProjectType::Branched {
                format!(
                    "Scopes: {} configured. Left/Right changes the selected scope.",
                    self.wizard.scopes.len()
                )
            } else {
        ...
        let active_probe = if self.wizard.project_type == ProjectType::Branched {
            self.wizard
                .current_scope()
                .and_then(|scope| scope.last_probe.as_ref())
        } else {
        ...
        if self.wizard.project_type == ProjectType::Branched {
            let validated = self
                ...
                .filter(|scope| {
                    matches!(
                        scope.last_probe.as_ref().map(|probe| probe.kind),
        ...
        if let Some(probe) = active_probe {
            ...
            lines.push(Line::from(
                "Use F5 or Read to inspect the selected target file.",
            ));
            lines.push(Line::from(
                "Save requires every branched scope to be read successfully.",
            ));
        }
    ...
    fn render_button_row(&mut self, frame: &mut Frame, area: Rect, buttons: &[DialogButton]) {
        ...
        for button in buttons {
            constraints.push(Constraint::Length(
                (button.label.chars().count() as u16 + 6).max(14),
            ));
            constraints.push(Constraint::Fill(1));
        }
        let chunks = Layout::default()
        ...
        for (index, button) in buttons.iter().enumerate() {
            ...
            self.hit_targets
                .push(HitTarget::new(rect, button.action.clone()));
        }
    }
    ...
    fn render_tag_annotation_dialog(&mut self, frame: &mut Frame, area: Rect) {
        ...
        frame.render_widget(
            Paragraph::new(
                "Enter inserts a new line. F2 or Ctrl+S saves the annotation. Esc cancels.",
            ),
            sections[0],
        );
    ...
    fn render_tag_annotation_editor(
        &self,
        frame: &mut Frame,
        area: Rect,
        dialog: &TagAnnotationDialog,
    ) {
        self.render_textarea_editor(
            frame,
            area,
    ...
    fn render_footer(&self, frame: &mut Frame, area: Rect) {
        ...
        let help = if self.help_modal.is_some() {
            ...
            {
                Line::from(
                    "Tab switch From/To | Left/Right change refs | Ctrl+S save preview | PgUp/PgDn or wheel scroll preview | Enter/F2/Esc close",
                )
            } else {
                Line::from(
                    "Type release notes | Ctrl+S save preview | F2 continue/close | PgUp/PgDn or wheel scroll preview | Esc cancel",
                )
            }
            ...
            Line::from(
                "1/2 switch tabs | [ ] scope | Left/Right history | Up/Down scroll | R reload | T create tag | Esc close",
            )
    ...
    fn render_browser_dialog(&mut self, frame: &mut Frame, area: Rect) {
        ...
        for (offset, _) in files[start..end].iter().enumerate() {
            ...
            self.hit_targets.push(HitTarget::new(
                rect,
                HitAction::BrowserSelect(start + offset),
            ));
        }
